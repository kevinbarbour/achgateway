...
func NewEnvironment(env *Environment) (*Environment, error) {
	...
	if env.PublicRouter == nil {
		...
		// append HTTP routes
		web.NewFilesController(env.Config.Logger, env.Config.Inbound.HTTP, httpFiles, fileRepository, fileReceiver.CancellationResponses).AppendRoutes(env.PublicRouter)
